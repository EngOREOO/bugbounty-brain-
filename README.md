...
| `skills/bb-intel-*/` | 8 writeup-intel skills distilled from 110+ real bug bounty writeups (auth/ATO, XSS chains, IDOR/logic, infoleak/recon, RCE/SQLi, cloud/edge, LLM code review, exploit chains) — techniques with verbatim payloads, verification steps, case index | — |
| `skills/` | 364 red-team & hunting skills harvested from curated X reposts (recon-skills, claude-bug-bounty, Anthropic-Cybersecurity-Skills, reverse-skill, VibeSec) — install any folder into your agent's skills root | — |
| `urgent-let-gaurd/` | Execution-guard skill: fail-closed AUTHORIZATION.md gate (no license = refusal, valid license = standing authorization till bounty) with validator + auth-file generator scripts | — |
| `ai-agent-bug-bounty-recon/` | Learned skill: AI-agent-orchestrated bug bounty recon on a VPS — skill files, scope-disciplined recon→injection pipeline, sqlite3 result inspection (distilled from video study) | — |
